...
public class _3x3_ extends Board implements EventHandler<ActionEvent>{
	...
	public _3x3_(Stage window, ColorTheme theme, Player p1, Player p2, Boolean aiMode) {
		
		//Initializing
		super(window, theme, p1, p2, aiMode, 3);
		super.aiMode = aiMode;
	...
	protected void displayCenterLayer() {
		...
		for(int row = 0; row < 3; row++) {
			...
			for(int column = 0; column < 3; column++) {
				...
				buttons[row][column].setMinSize(150, 150);
				buttons[row][column].setPrefSize(150, 150);
				buttons[row][column].setMaxSize(150, 150);
				...
				//Set ID
				buttons[row][column].setId("By3x3Grid");
	...
	protected void check() {
		...
		/**
		 * Check Diagonal Match Condition
		 */
		...
		if(
		   ...
		   (buttons[2][0].getText().equals("O"))
			...
			playerTwoWins(2, 1, 0, new int[] {0, 1, 2});
			...
			Timeline timeline = new Timeline(new KeyFrame(Duration.seconds(2), e -> {
				...
				try {
					if(!this.player1_turn) {
						...
						int ai_row = random.nextInt(3);
						int ai_col = random.nextInt(3);
						
						while(!buttons[ai_row][ai_col].getText().equals("")) {
							ai_row = random.nextInt(3);
							ai_col = random.nextInt(3);
